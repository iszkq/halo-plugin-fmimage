...
- 插件版本：`1.0.30`
- 版本规则：后续只递增最后一位，例如 `1.0.30 -> 1.0.31`
...
- 新增免费模型：`gemini-3.1-flash-image-preview-free`
- 按 AiHubMix 的 OpenAI 兼容调用方式接入 Gemini 图片生成
- 解析器新增兼容 `choices -> message -> multi_mod_content -> inline_data`
- 前端会根据模型切换“尺寸”或“宽高比”选项
- 补了单测，固定 Gemini 这类返回结构的解析行为
