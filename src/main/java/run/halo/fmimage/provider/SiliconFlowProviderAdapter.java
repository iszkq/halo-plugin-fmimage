...
@Component
public class SiliconFlowProviderAdapter extends AbstractJsonProviderAdapter {
    public SiliconFlowProviderAdapter(WebClient.Builder webClientBuilder) {
        super(webClientBuilder);
    }

    @Override
    public boolean supports(ProviderType providerType) {
        return providerType == ProviderType.SILICONFLOW;
    }
    ...
    public Mono<UpstreamImageResult> generate(ProviderType providerType, GenerateImageRequest request,
        ...
        payload.put("image_size", requestSize(request, generalSettings));
        payload.put("batch_size", requestCount(request, generalSettings));
        ...
        return postJson(imagesGenerationsUri(), payload, providerConfig, generalSettings)
            .map(root -> {
                var items = parseFlexibleOutputItems(root.path("images"), "image/png");
                if (items.isEmpty()) {
